...
func runTronGenerateC(ctx context.Context, parser *gcmd.Parser) (err error) {
	...
	// 开始并发处理
	err = processAddressesWithConcurrencyC(ctx, []string{address}, 0, maxDepth, processedAddresses, &totalProcessed, &totalInserted, &mu)
	if err != nil {
		return err
	}
...
// 并发处理一批地址
func processAddressesWithConcurrencyC(
	ctx context.Context,
	addresses []string,
	currentDepth,
	maxDepth int,
	...
	for _, addr := range addresses {
		...
		go func(address string) {
			...
			*totalProcessed += len(addresses)
			...
			result, err := insertOrIgnoreAddressesC(ctx, addresses)
			if err != nil {
				fmt.Printf("插入地址失败: %v\n", err)
				return
			}
	...
	if currentDepth < maxDepth-1 && len(nextLevelAddresses) > 0 {
		...
		// 递归处理下一层地址
		return processAddressesWithConcurrencyC(
			ctx,
			nextLevelAddresses,
			currentDepth+1,
			maxDepth,
